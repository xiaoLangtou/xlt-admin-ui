...
export const defaultResponseAdapter = <T>(response: unknown): ApiResponse<T> => {
  ...
  // 处理 ResPage 嵌套 pager 结构
  for (const field of tableConfig.pagerFields) {
    if (field in res && typeof res[field] === 'object' && res[field] !== null) {
      const pager = res[field] as Record<string, unknown>
      if (total === 0 || total === records.length) {
        total = extractTotal(pager, records, tableConfig.totalFields)
      }
      pagination = extractPagination(pager) ?? pagination
      break
    }
  }
